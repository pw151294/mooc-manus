...
func (s *BaseAgentApplicationServiceImpl) Chat(clientRequest dtos.ChatClientRequest, writer http.ResponseWriter) {
	...
	for {
		...
		case event, ok := <-eventCh:
			...
			event.SaveMessageId(messageId) // 注入 messageId 到事件 payload，供前端"停止对话"按钮定位
		...
		case <-timer.C:
			...
			timeoutEvent.SaveMessageId(messageId)
			sse.SendEvent(timeoutEvent, messageId)
			cancel() // 通知 agent 中止
...
func (s *BaseAgentApplicationServiceImpl) stopMessageInternal(messageId string) dtos.StopMessageCleanDetail {
	...
	if hasPending {
		_ = slot.resolve(interrupt.Decision{Kind: interrupt.DecisionCancel})

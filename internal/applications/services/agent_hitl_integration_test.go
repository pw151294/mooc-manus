...
func buildAgent(mid string, sink interrupt.PendingSink, tool tools.Tool) *agents.BaseAgent {
...
func TestHITL_I07_StopCancelsPending(t *testing.T) {
	...
	svc.mu.Unlock()
	if !slot.resolve(interrupt.Decision{Kind: interrupt.DecisionCancel}) {
...
func TestHITL_I08_DoubleRegisterRejected(t *testing.T) {
	svc := newTestSvc(2 * time.Second)
	_, err := svc.RegisterInterrupt("mid-i08", interrupt.Snapshot{ToolCallID: "tc1"})
	if err != nil {
		t.Fatalf("first register err = %v", err)
	}
	_, err = svc.RegisterInterrupt("mid-i08", interrupt.Snapshot{ToolCallID: "tc2"})
...
func TestHITL_I10_TimerBeatsResume(t *testing.T) {
	...
	ch, err := svc.RegisterInterrupt("mid-i10", interrupt.Snapshot{ToolCallID: "tc1"})
	if err != nil {
	...
	select {
	case d := <-ch:
		if d.Kind != interrupt.DecisionTimeout {
...
func TestHITL_I11_ResumeBeatsTimer(t *testing.T) {
	...
	ch, err := svc.RegisterInterrupt("mid-i11", interrupt.Snapshot{ToolCallID: "tc1"})
	if err != nil {
		t.Fatalf("Register err = %v", err)
	}
	// 后台消费者：收到 approve 后计数
	got := make(chan interrupt.Decision, 2)
	go func() {
		for d := range ch {
			got <- d
		}
...
drain:
	for {
		...
		case d, ok := <-got:
			...
			count++
			if d.Kind != interrupt.DecisionApprove {
...
func TestHITL_I12_ConcurrentResumeFirstWins(t *testing.T) {
	svc := newTestSvc(5 * time.Second)
	ch, err := svc.RegisterInterrupt("mid-i12", interrupt.Snapshot{ToolCallID: "tc1"})
	if err != nil {
		t.Fatalf("Register err = %v", err)
	}
	// 消费 ch 防止阻塞
...
// -----------------------------------------------------------------------------
// 注：原 I-13（pendingSink 为 nil 时跳过闸门）已随架构决策删除
// —— HITL 已作为 BaseAgent 固有能力集成，pendingSink 不再允许为 nil
// -----------------------------------------------------------------------------

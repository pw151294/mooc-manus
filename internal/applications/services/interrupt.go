...
type pendingSlot struct {
	snapshot interrupt.Snapshot
	ch       chan interrupt.Decision
	resolved atomic.Bool
	timer    *time.Timer
}

// resolve 保证 chan 只被写入一次；返回 true 表示这次决策生效
func (p *pendingSlot) resolve(d interrupt.Decision) bool {
...
func (s *BaseAgentApplicationServiceImpl) RegisterInterrupt(
	messageId string, snap interrupt.Snapshot,
) (<-chan interrupt.Decision, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	slot.timer = time.AfterFunc(s.waitTimeout, func() {
		...
		s.mu.Unlock()
		if ok && cur == slot {
			_ = slot.resolve(interrupt.Decision{Kind: interrupt.DecisionTimeout})
		}
	})
...
func (s *BaseAgentApplicationServiceImpl) Resume(req dtos.ResumeClientRequest) dtos.ResumeResult {
	...
	d := interrupt.Decision{
		Kind:     interrupt.DecisionKind(req.Decision),

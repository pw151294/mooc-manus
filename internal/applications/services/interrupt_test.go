...
func newTestSlot() *pendingSlot {
	return &pendingSlot{
		snapshot: interrupt.Snapshot{ToolCallID: "tc1"},
...
func TestPendingSlot_ResolveFirstWins(t *testing.T) {
	slot := newTestSlot()
	if !slot.resolve(interrupt.Decision{Kind: interrupt.DecisionApprove}) {
		t.Fatal("first resolve should return true")
	}
	if slot.resolve(interrupt.Decision{Kind: interrupt.DecisionReject}) {
...
func TestPendingSlot_ConcurrentResolve(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(i int) {
			defer wg.Done()
			kind := interrupt.DecisionApprove
			if i%2 == 0 {
				kind = interrupt.DecisionReject
			}
			if slot.resolve(interrupt.Decision{Kind: kind}) {
...
func TestRegisterInterrupt_Success(t *testing.T) {
	...
	ch, err := s.RegisterInterrupt("m1", interrupt.Snapshot{ToolCallID: "tc1"})
	if err != nil || ch == nil {
...
func TestRegisterInterrupt_AlreadyPending(t *testing.T) {
	s := newTestSvc(time.Minute)
	_, _ = s.RegisterInterrupt("m1", interrupt.Snapshot{ToolCallID: "tc1"})
	_, err := s.RegisterInterrupt("m1", interrupt.Snapshot{ToolCallID: "tc2"})
...
func TestRegisterInterrupt_TimerFires(t *testing.T) {
	...
	ch, _ := s.RegisterInterrupt("m1", interrupt.Snapshot{ToolCallID: "tc1"})
	select {
	case d := <-ch:

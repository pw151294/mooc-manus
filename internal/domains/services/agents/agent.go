...
func (s *BaseAgentDomainServiceImpl) createBaseAgent(request agents.ChatRequest) (*BaseAgent, error) {
	...
	// messageId 透传供 fileEdit 隔离临时 workspace 与 bashExec audit 关联
	// conversationId 透传供 fileEdit persistent=true 时定位持久化规划目录
	nativeTools, err := tools.NativeTools(s.nativeToolsProvider, request.MessageId, request.ConversationId)
	if err != nil {
	...
	return NewBaseAgent(appConfig.AgentConfig, inv, chatMemory, baseTools, systemPrompt,
		...
		WithPendingSink(request.PendingSink),
	), nil
}

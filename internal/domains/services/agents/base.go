...
func (a *BaseAgent) InvokeToolCalls(ctx context.Context, toolCalls []llm.ToolCall, eventCh chan<- events.AgentEvent) []llm.Message {
	...
	for _, toolCall := range toolCalls {
		...
		func() {
			...
			if tool.SupportsRiskAssessment() {
				...
				if perr != nil {
					...
					if regErr != nil {
						...
						toolSpan.SetError(regErr)
						toolSpan.AddLog("ERROR", "tool.hitl.register_failed", map[string]interface{}{"message": regErr.Error()})
						failCount++
						abort = true
						return
					}
					...
					case interrupt.DecisionApprove:
						toolSpan.SetTag("tool.hitl.decision", "approve")
						toolSpan.AddLog("INFO", "tool.hitl.decided", map[string]interface{}{"decision": "approve"})
					...
					case interrupt.DecisionReject:
						...
						toolMessages = appendSiblingSkipped(toolMessages, toolCalls, toolCall.ID)
						a.circuitBreaker.StartNewRound(currentRoundKeys)
						toolSpan.SetTag("tool.hitl.decision", "reject")
						toolSpan.AddLog("INFO", "tool.hitl.decided", map[string]interface{}{"decision": "reject"})
						failCount++
						abort = true
						return
					case interrupt.DecisionTimeout:
						...
						toolMessages = appendSiblingSkipped(toolMessages, toolCalls, toolCall.ID)
						a.circuitBreaker.StartNewRound(currentRoundKeys)
						toolSpan.SetTag("tool.hitl.decision", "timeout")
						toolSpan.AddLog("INFO", "tool.hitl.decided", map[string]interface{}{"decision": "timeout"})
						failCount++
						abort = true
						return
					case interrupt.DecisionCancel:
						toolSpan.SetTag("tool.hitl.decision", "cancel")
						toolSpan.AddLog("INFO", "tool.hitl.decided", map[string]interface{}{"decision": "cancel"})
			...
			toolSpan.SetTag("tool.provider", tool.ProviderName())
			if !result.Success {
				a.recordToolFailure(key, funcName, funcArgs)
				...
				toolSpan.AddLog("ERROR", "tool.invoke.failed", map[string]interface{}{"message": result.Message})
				failCount++
			} else {
			...
			toolSpan.AddLog("INFO", "tool.invoke.completed", map[string]interface{}{"success": result.Success})
		}()

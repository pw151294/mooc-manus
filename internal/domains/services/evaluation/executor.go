...
func (e *InstanceExecutor) Execute(ctx context.Context, instanceID string) error {
	...
	if err != nil {
		e.logger.Error("EVAL_STAGE_INIT_CAS_ERR",
	...
	if !ok {
		e.logger.Warn("EVAL_STAGE_INIT_CAS_MISS",
			zap.String("instance_id", instanceID),
			zap.String("worker_id", e.workerID),
	...
	if err != nil {
		e.logger.Error("EVAL_STAGE_INIT_LOAD_ERR",
			zap.String("instance_id", instanceID),
			zap.Error(err))
		return err
	}
	e.logger.Info("EVAL_STAGE_INIT",
...
func (e *InstanceExecutor) executeStages(ctx context.Context, inst *ev.RunInstance) error {
	...
	if inst.CaseSnapshot.InitScript != "" {
		...
		e.logger.Info("EVAL_STAGE_INIT_SCRIPT_DONE",
			zap.String("instance_id", inst.ID),
			zap.String("case_id", inst.CaseID),
	...
	if err != nil {
		e.logger.Error("EVAL_STAGE_SNAPSHOT_LOAD_ERR",
			zap.String("instance_id", inst.ID),
	...
	if !ok {
		e.logger.Warn("EVAL_STAGE_RUN_CAS_MISS",
			zap.String("instance_id", inst.ID),
	...
	chatStart := time.Now()
	e.logger.Info("EVAL_STAGE_RUN_ENTER",
		zap.String("instance_id", inst.ID),
		zap.String("task_id", inst.TaskID),
		zap.String("case_id", inst.CaseID),
		zap.String("conversation_id", inst.ConversationID),
	...
	if cerr != nil {
		e.logger.Error("EVAL_STAGE_RUN_ERR",
			zap.String("instance_id", inst.ID),
	...
	if chatRes.DidTimeout {
		e.logger.Warn("EVAL_STAGE_RUN_TIMEOUT",
			zap.String("instance_id", inst.ID),
			zap.Int64("duration_ms", chatDur),
	...
	if chatRes.Error != nil {
		e.logger.Warn("EVAL_STAGE_RUN_AGENT_ERR",
			zap.String("instance_id", inst.ID),
			zap.Int64("duration_ms", chatDur),
	...
	e.logger.Info("EVAL_STAGE_RUN_DONE",
		zap.String("instance_id", inst.ID),
		zap.Int64("duration_ms", chatDur),
	...
	if !ok {
		e.logger.Warn("EVAL_STAGE_VERIFY_CAS_MISS",
			zap.String("instance_id", inst.ID),
	...
	e.logger.Info("EVAL_STAGE_VERIFY_DONE",
		zap.String("instance_id", inst.ID),
		zap.String("case_id", inst.CaseID),
...
func (e *InstanceExecutor) finalizeError(ctx context.Context, inst *ev.RunInstance,
	...
	if !ok {
		e.logger.Warn("EVAL_STAGE_FINALIZE_ERROR_CAS_MISS",
			zap.String("instance_id", inst.ID),
	...
	if err := e.resultRepo.Upsert(ctx, &ev.Result{
		...
		e.logger.Error("EVAL_STAGE_FINALIZE_ERROR_RESULT_UPSERT_ERR",
			zap.String("instance_id", inst.ID),
			zap.Error(err))
	}
	if err := e.taskRepo.RecountAndTransit(ctx, inst.TaskID); err != nil {
		e.logger.Warn("EVAL_STAGE_FINALIZE_ERROR_RECOUNT_ERR",
			zap.String("task_id", inst.TaskID),
			zap.Error(err))
	}
	e.logger.Warn("EVAL_STAGE_FINALIZE_ERROR",
		zap.String("instance_id", inst.ID),
		zap.String("task_id", inst.TaskID),
		zap.String("case_id", inst.CaseID),
		zap.String("from", string(from)),
...
func (e *InstanceExecutor) finalizeVerify(ctx context.Context, inst *ev.RunInstance,
	...
	if e.aggregator != nil {
		...
		metrics = m
		if metrics != nil {
			e.logger.Info("EVAL_STAGE_AGGREGATE",
				zap.String("instance_id", inst.ID),
				zap.String("conversation_id", inst.ConversationID),
	...
	if !casOK {
		e.logger.Warn("EVAL_STAGE_FINALIZE_CAS_MISS",
			zap.String("instance_id", inst.ID),
	...
	if err := e.resultRepo.Upsert(ctx, result); err != nil {
		e.logger.Error("EVAL_STAGE_FINALIZE_RESULT_UPSERT_ERR",
			zap.String("instance_id", inst.ID),
			zap.Error(err))
	}
	if err := e.taskRepo.RecountAndTransit(ctx, inst.TaskID); err != nil {
		e.logger.Warn("EVAL_STAGE_FINALIZE_RECOUNT_ERR",
			zap.String("task_id", inst.TaskID),
			zap.Error(err))
	}
	e.logger.Info("EVAL_STAGE_FINALIZE",
		zap.String("instance_id", inst.ID),
		zap.String("task_id", inst.TaskID),
		zap.String("case_id", inst.CaseID),

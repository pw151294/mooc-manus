...
func NewRunInstanceHandler(
	...
	return &RunInstanceHandler{
		...
		instRepo: instRepo,
		caseGate: caseGate,
		logger:   logger.Zap().Named("eval.mq"),
	}
}
...
func (h *RunInstanceHandler) ProcessTask(ctx context.Context, task *asynq.Task) error {
	...
	if err := p.Unmarshal(task.Payload()); err != nil {
		h.logger.Error("EVAL_MQ_PAYLOAD_UNMARSHAL_ERR",
	...
	h.logger.Info("EVAL_MQ_CONSUME_START",
	...
	if err != nil {
		h.logger.Error("EVAL_MQ_INSTANCE_MISS",
			zap.String("instance_id", p.InstanceID),
			zap.Error(err))
		return errors.Join(asynq.SkipRetry, err)
	}
	...
	if err != nil {
		...
		h.logger.Warn("EVAL_MQ_TOKEN_ACQUIRE_ERR",
			zap.String("instance_id", p.InstanceID),
	...
	if !ok {
		...
		h.logger.Info("EVAL_MQ_TOKEN_BUSY",
			zap.String("instance_id", p.InstanceID),
			zap.String("case_id", inst.CaseID),
	...
	defer func() {
		...
		if rerr := h.caseGate.Release(ctx, inst.CaseID); rerr != nil {
			h.logger.Warn("EVAL_MQ_TOKEN_RELEASE_ERR",
				zap.String("instance_id", p.InstanceID),
				zap.String("case_id", inst.CaseID),
				zap.Error(rerr))
		}
	}()
	h.logger.Info("EVAL_MQ_TOKEN_ACQUIRED",
		zap.String("instance_id", p.InstanceID),
		zap.String("case_id", inst.CaseID))

	execErr := h.executor.Execute(ctx, p.InstanceID)
	h.logger.Info("EVAL_MQ_CONSUME_DONE",
		zap.String("instance_id", p.InstanceID),
		zap.String("case_id", inst.CaseID),

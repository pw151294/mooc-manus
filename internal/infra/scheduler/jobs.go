...
func NewSweeperJob(d evalsvc.EvaluationDomainService, l *zap.Logger) *SweeperJob {
	if l == nil {
		l = zap.NewNop()
	}
	return &SweeperJob{domain: d, logger: l}
}
...
func (j *SweeperJob) Run() {
	...
	if err != nil {
		j.logger.Warn("sweep stale instances 失败", zap.Error(err))
		return
	}
	if n > 0 {
		j.logger.Info("sweep stale instances", zap.Int("count", n))
	}
}
...
func NewReconcilerJob(d evalsvc.EvaluationDomainService, l *zap.Logger) *ReconcilerJob {
	if l == nil {
		l = zap.NewNop()
	}
	return &ReconcilerJob{domain: d, logger: l}
}
...
func (j *ReconcilerJob) Run() {
	...
	if err != nil {
		j.logger.Warn("reconcile task statuses 失败", zap.Error(err))
		return
	}
	if n > 0 {
		j.logger.Info("reconcile task statuses", zap.Int("count", n))
	}
}
...
func NewDLQArchiverJob(d evalsvc.EvaluationDomainService, l *zap.Logger) *DLQArchiverJob {
	if l == nil {
		l = zap.NewNop()
	}
	return &DLQArchiverJob{domain: d, logger: l}
}
...
func (j *DLQArchiverJob) Run() {
	...
	if err != nil {
		j.logger.Warn("archive dead tasks 失败", zap.Error(err))
		return
	}
	if n > 0 {
		j.logger.Info("archive dead tasks", zap.Int("count", n))
	}
}

...
func (j *SweeperJob) Run() {
	...
	if err != nil {
		logger.Zap().Named("eval").Warn("sweep stale instances 失败", zap.Error(err))
		return
	}
	if n > 0 {
		logger.Zap().Named("eval").Info("sweep stale instances", zap.Int("count", n))
	}
}
...
func (j *ReconcilerJob) Run() {
	...
	if err != nil {
		logger.Zap().Named("eval").Warn("reconcile task statuses 失败", zap.Error(err))
		return
	}
	if n > 0 {
		logger.Zap().Named("eval").Info("reconcile task statuses", zap.Int("count", n))
	}
}
...
func (j *DLQArchiverJob) Run() {
	...
	if err != nil {
		logger.Zap().Named("eval").Warn("archive dead tasks 失败", zap.Error(err))
		return
	}
	if n > 0 {
		logger.Zap().Named("eval").Info("archive dead tasks", zap.Int("count", n))
	}
}
